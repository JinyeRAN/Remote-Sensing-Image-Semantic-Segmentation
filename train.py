...
def get_loader(args):
    ...
    train_transform = MyTransform(reshape_size=(256,256),crop_size=(256,256), augment=True)  # data transform for training set with data augmentation, including resize, crop, flip and so on
    val_transform = MyTransform(reshape_size=(256,256),crop_size=(256,256), augment=False)   #data transform for validation set without data augmentation
...
def train(args, model):
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        # net1 = torch.nn.DataParallel(model.train(),device_ids=[0,1])
        net1 = model.cuda().train()
        count = 1
        ...
        average_epoch_loss_val, iouVal = eval(args, net1, loader_val, criterion, epoch)     
        
        #save model every X epoch
        if  epoch % args.epoch_save==0:
            torch.save(net1.state_dict(), '{}_{}.pth'.format(os.path.join(args.savedir,args.model),str(epoch)))
